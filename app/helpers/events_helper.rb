module EventsHelper
  ...
  def link_to_show_event(link_text, event)
    link_to link_text, event_path(event)
  end
  ...
  def link_to_xls_export
    link_to 'Export list to XLS', '#',
               :onclick => "var url=location.href; url=url.replace('/events','/events.xls'); window.location=url"
  end
